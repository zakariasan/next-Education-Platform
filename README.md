...
- Attendance: manual or automatic? → [Attendance](#attendance-manual-not-automatic)
...
3. **Classes** → `Create a Class` (name, description, school). Open a class:
   ...
   - **Quizzes** tab — MCQ quizzes linked to lessons.
   - **Exams** (`/classes/[id]/exams`) — exam with statement/correction/rubric files, per-student score → XP (`maxXP × score/maxScore`).
   - **Séances** (`/classes/[id]/seances`) — attendance sessions, see [Attendance](#attendance-manual-not-automatic).
...
6. **Students** — attendance/XP/level table across your classes. **Events** — school/class calendar.

## Student walkthrough

Log in as `alice@physiclub.demo`. Sidebar: **Home · Holy Graph · Peer Reviews**.
...
3. **Peer Reviews** — submissions from classmates on projects you already validated (only their `Peer` criteria and answers are visible). Grading earns XP and a correction point.
...
## Attendance: manual, not automatic

There is **no automatic check-in** (no QR code, geolocation, or student self-check-in). The teacher records attendance by hand:

1. Class → **Séances** → `Create` a session (title, start, end). The creation form lists the class's students with an attendance status each; saving creates the initial `SeanceParticipation` rows (`POST /api/teacher/classes/[id]/seances/parte`).
2. Open the séance: each student has a status dropdown **PRESENT / ABSENT / LATE / EXCUSED** and an **XP** field; changes save immediately (`PATCH …/seances/[seanceId]/attendance`) and the XP difference is applied to the student's `totalXP`.
3. Attendance rate and XP feed the teacher's **Students** page and the student's home chart. `ClassEnrollment.presentStreak` exists in the schema for a future consecutive-presence bonus but nothing writes to it yet.
